...
- **版本升级**: 从 v2.1.0 升级至 v3.0.1。
- **部署简化**: 新增一键安装脚本，普通用户无需配置即可完成安装部署。
...
- **去除阿里云盘功能**:
  - **移除配置项与播放器入口**: 移除阿里云盘相关配置、播放器音源入口及全部后端接口，简化部署与维护。
- **本地音乐播放 Range 请求修复**:
  - **Suffix Range 支持**: 支持 `bytes=-N` 后缀范围请求，修复部分播放器发送 suffix Range 请求时本地音乐无法正常播放的问题。
  - **越界与非法校验**: 对越界、逆序、非法 Range 正确返回 416，避免服务端异常。
- **内网地址挂载修复 (WebDAV / OpenList)**:
  - **无协议地址自动补全**: 配置 `192.168.1.100:5005` 这类未带协议的裸地址时自动补全 `http://`，修复内网 NAS 的 WebDAV 与 OpenList 无法挂载的问题。
- **OpenList 测试连接错误诊断增强**:
  - **区分登录失败与路径错误**: 测试连接时单独报告账号登录失败原因（如用户名或密码错误），不再静默降级为游客访问导致报错误导。
